...
date: "2022-08-26"
category: "ssafy"
period: "2022.07 ~ 2022.08 (약 7주)"
team: "6인"
contribution: "프론트엔드 파트 리더"
role: "프론트엔드"
...
- 가상 컬러 드레이프 오버레이 (패브릭 이미지 오버레이 + CSS 필터)
- 밝기·채도·색조 조절 컨트롤
- OpenVidu 시그널링 기반 실시간 채팅
...
| 분류        | 기술                                                                           |
| ----------- | ------------------------------------------------------------------------------ |
| Frontend    | React 18, JavaScript, Redux Toolkit, redux-persist, Material UI (Emotion 엔진) |
| 실시간 통신 | OpenVidu 2.x (WebRTC), OpenVidu 시그널링 (채팅·필터 동기화)                    |
...
- **컬러 드레이프 시스템**: 패브릭 이미지 오버레이 + CSS 필터 기반 컬러 필터, RGB 변환 유틸리티, Canvas 드로잉(react-canvas-draw) 주석 도구
- **컬러 팔레트 UI**: 시즌별 컬러셋 선택·표시 컴포넌트, Best/Worst 컬러 인터페이스

## 회고

화상 통화를 처음 다뤄본 프로젝트입니다. 시그널링 서버를 직접 세우면 7주 안에 끝나지 않을 것 같아 OpenVidu를 썼고, 아낀 시간은 통화 화면 위에 컬러 드레이프를 얹는 쪽에 몰았습니다. 패브릭 이미지를 CSS 필터로 겹치는 방식이라 영상 위에서 색이 어떻게 보이는지 맞추는 데 손이 가장 많이 갔습니다. 채팅도 별도 서버를 두지 않고 OpenVidu 시그널링에 얹어 해결했습니다.

프론트엔드 파트 리더를 맡아 3인이 Feature 단위로 작업을 나눠 가도록 폴더를 갈랐고, Redux 슬라이스 형태를 하나로 통일했습니다. 같은 화면을 여러 명이 동시에 건드릴 때 충돌을 줄이는 것이 목적이었습니다.
