...
정보보안기사 자격증 시험을 준비하는 사용자를 위한 학습 플랫폼입니다. 필기 5과목 전 범위의 이론 콘텐츠와 760개의 객관식 문제, 그리고 실기(약어/용어·서술형·실무형) 콘텐츠까지 제공하며, 오답 노트와 모의고사 기능으로 효율적인 시험 대비를 지원합니다.
...
- 약어/용어·서술형·실무형 콘텐츠를 별도 페이지로 분리 제공
- 필기와 동일한 콘텐츠 파이프라인 위에 TS 데이터 파일로 실기 출제 유형 대응
...
src/
├── content/
│   ├── theory/          # 필기 이론 MDX 콘텐츠 (5과목)
│   ├── questions/       # 문제 JSON 데이터 (5과목)
│   └── practical/       # 실기 콘텐츠 (약어/용어·서술형·실무형)
├── pages/               # 페이지 컴포넌트 (13개)
...
- **SEO**: 빌드 후 50여 개 라우트별 정적 HTML 생성 스크립트 개발 (소셜 미디어 미리보기 대응)
...
SPA를 GitHub Pages에 배포하면 소셜 미디어 크롤러가 메타태그를 읽지 못합니다. 빌드 후 소스 파일에서 메타데이터를 추출하여 50여 개 라우트별(베이스·과목·챕터 단위) 정적 HTML을 생성하는 스크립트를 작성하여 OG 태그 문제를 해결했습니다.

## 회고

제가 시험을 준비하면서 필요한 것을 그때그때 만든 프로젝트입니다. 이론을 읽고, 문제를 풀고, 틀린 것만 다시 보고, 모의고사로 점검하는 순서 그대로 화면을 붙여 나갔습니다.

서버를 두지 않은 것은 오프라인에서도 쓰기 위해서였습니다. 정적 SPA로 빌드해 GitHub Pages에 올리고 PWA로 설치되게 하니 네트워크가 끊겨도 문제 풀이가 이어졌고, 오답은 localStorage에 쌓이니 계정도 필요 없었습니다.

콘텐츠는 MDX로 관리해서 보안 프로토콜 흐름 같은 것은 Mermaid로 그렸습니다. 문제를 데이터로만 두지 않고 이론과 같은 형식으로 다룬 덕분에 과목을 늘릴 때 손이 덜 갔습니다.
