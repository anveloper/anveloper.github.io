...
사용자: "미니멀한 흑백 명함 만들어줘"
    │
    ▼
[Stage 1] Orchestrator (Claude Sonnet)
    │ LLM이 프롬프트에서 생산 타입 분류
    │ → pod_type: "namecard"
    │ → search_query: "미니멀 흑백 비즈니스 카드"
    │
    ▼
[Stage 2] Retriever (OpenAI Embedding + pgvector)
    │ WHERE "POD_TYPE" = 'namecard' 파티션만 검색
    │ → 전체 2,000만 건 중 ~200만 건만 스캔
    │
    ▼
유사 템플릿 top-k 반환
```
...
```
Phase 1 (100만 건)
  → 파티셔닝 + 2단계 검색 + halfvec HNSW
  → API ~$22.5, 월 인프라 ~25만 원
