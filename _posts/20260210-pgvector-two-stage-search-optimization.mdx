...
pgvector는 PostgreSQL에 벡터 유사도 검색 기능을 추가하는 오픈소스 확장이다. 전용 벡터 DB(Pinecone, Milvus, Qdrant)와 달리, **기존 PostgreSQL 인프라에서 관계형 데이터와 벡터를 통합 관리**할 수 있다는 것이 최대 장점이다. 별도 인프라 없이 JOIN, 트랜잭션, 인덱스 등 PostgreSQL의 모든 기능을 벡터 데이터와 함께 사용할 수 있다.

### 인덱스: HNSW vs IVFFlat

pgvector는 두 가지 근사 최근접 이웃(ANN) 인덱스를 지원한다.

| 항목 | HNSW | IVFFlat |
|------|------|---------|
| 검색 속도 | 빠름 (speed-recall 트레이드오프 우수) | 보통 |
| 빌드 속도 | 느림 | 빠름 |
| 메모리 사용 | 높음 | 낮음 |
| 데이터 없이 빌드 | 가능 | 불가 (학습 단계 필요) |
| 동적 데이터 | 강함 (삽입/삭제 영향 적음) | 약함 (재빌드 권장) |

v0.7.0에서 도입된 `halfvec`(float16) 타입은 벡터 저장 공간을 **50% 절감**하면서도 검색 품질(recall) 손실이 거의 없다. 또한 `halfvec`은 HNSW에서 **4000차원**까지 지원하여, `vector`(float32)의 2000차원 제한을 넘어 대형 임베딩 모델(OpenAI 3072차원 등)과의 호환성이 높다.

v0.8.0에서는 **반복 스캐닝**(iterative scan)이 추가되어, 필터 조건과 벡터 검색을 결합할 때 결과가 부족한 "오버필터링" 문제를 해결했다. `hnsw.iterative_scan`과 `ivfflat.iterative_scan` 파라미터로 활성화할 수 있다.

이 글에서는 pgvector의 HNSW + halfvec에 PostgreSQL 파티셔닝과 LLM 사전 분류를 결합한 **2단계 검색 전략**으로, 2,000만 건 규모에서 인프라 비용을 1/6로 줄인 과정을 정리한다.
