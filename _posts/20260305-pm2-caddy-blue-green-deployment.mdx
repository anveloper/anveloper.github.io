...
```
GitHub (release push) → Self-hosted Runner
...
3. deploy.sh 실행
   ├── 현재 활성 포트 확인 (예: blue:4033)
   ├── git pull + pnpm install
   ├── NEXT_DIST_DIR=.next-green pnpm build  ← 기존 서비스 영향 없음
   ├── pm2 restart my-store-green
   ├── 헬스체크 (localhost:4034)
   ├── Caddy upstream 전환 → 4034
   └── 완료 (green 활성, blue 대기)
4. 다음 배포 시 반대로 (green → blue)
```
