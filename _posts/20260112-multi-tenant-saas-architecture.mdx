...
여기에 기존 본사 API와 연동하여 상품·주문 정보를 실시간으로 동기화해야 하는 요구사항도 있었다. 이 글에서는 이 시스템의 멀티테넌트 아키텍처를 어떻게 설계했는지 정리한다.
...
```mermaid
flowchart LR
  Client[클라이언트] <--> Store[스토어 앱]
  Store <--> API[본사 API]
  Store <--> DB["MariaDB\n(설정/참조 정보만)"]
```

DB에는 테넌트 설정, 페이지 레이아웃, 고객 정보, 장바구니, 주문 메타데이터만 저장하고, 상품명·가격·이미지 같은 실제 상품 정보는 본사 API에서 실시간으로 조회한다.

단, 주문 시점의 상품명과 가격은 스냅샷으로 저장한다. 나중에 상품 정보가 바뀌더라도 주문 당시의 정보를 보존하기 위해서다.

```prisma
model OrderItem {
  extProductSeqno Int     // 본사 상품 참조키
...
전체 스키마는 23개 모델로 구성된다. 핵심 관계를 요약하면 다음과 같다.
...
graph TD
  ...
  Order --> Payment[Payment 결제]
  Order --> NotificationLog[NotificationLog 알림 로그]
  Tenant --> File[File 파일]
  Tenant --> NotificationRecipient[NotificationRecipient 알림 수신자]
  ...
  VerificationCode[VerificationCode 인증 코드]
```

초기 설계 시 16개 테이블로 시작했지만, 결제(Payment), 알림(NotificationRecipient, NotificationLog), 인증(VerificationCode) 등의 기능이 추가되면서 23개로 성장했다. 테넌트별 데이터 격리는 모든 주요 테이블에 `tenantId` 외래키를 통해 구현했다.
