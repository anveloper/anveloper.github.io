---
title: '멀티테넌트 SaaS 아키텍처 설계기 — DPS Store'
date: '2026-01-12'
...
```
Tenant (테넌트)
  ├── Theme (테마: 로고, 컬러, 공통 스타일)
  ├── Page (페이지: 레이아웃, 스타일)
  │    └── PageFile (페이지-파일 N:N)
  ├── Product (상품 설정)
  │    ├── ProductCategory (상품-카테고리 N:N)
  │    └── ProductDisplay (상품-전시 N:N)
  ├── Category (카테고리)
  ├── Display (전시)
  ├── Terms (약관)
  ├── TenantFont (테넌트-폰트 N:N)
  ├── Cart → CartItem (장바구니)
  ├── Order → OrderItem (주문)
  └── File (파일)
