...
| ORM / DB   | Prisma 7 + `@prisma/adapter-neon` + Vercel Postgres (Neon) |
| Auth       | NextAuth 5 (어드민 한정)                                   |
| Validation | Zod 4                                                      |
| Styling    | Tailwind CSS 4                                             |
| 모션 / 3D  | motion, three.js, postprocessing                           |
| Storage    | Vercel Blob (제출 자료 업로드)                             |

DB는 Neon 서버리스 어댑터를 끼워 `lib/prisma.ts` 싱글톤으로 묶었다. dev 핫리로드에서 클라이언트가 중복 생성되는 이슈를 막기 위함이다. 폼 입력은 모두 Zod 스키마(`lib/validations`)를 통해 1차 검증한 뒤 Server Action으로 흘렸고, Route Handler는 디스코드 봇/AI 심사 같은 외부 호출 진입점으로만 남겼다.
...
- 홈페이지 제작 (랜딩, 갤러리, 결과 페이지)
- Prisma 스키마 설계 및 마이그레이션
- 참가 신청 폼 로직 구성 (Zod 검증 + Server Action)
- 프로젝트 관리 어드민 (NextAuth 보호)
- 실시간 좋아요 기능 (서버 폴링)
- 디스코드 봇 연동을 위한 `/api/evaluate` 엔드포인트
...
<div align="center">
  <img
    src="/images/posts/ttalkkakthon-organizer-retrospective/002.webp"
    alt="해커톤 굿즈 — 키캡, 일수 찌라시 스타일 스티커"
    width="360"
  />
</div>
...
<div align="center">
  <img
    src="/images/posts/ttalkkakthon-organizer-retrospective/003.webp"
    alt="용기의 천사링을 착용하고 발표 중"
    width="480"
  />
</div>
...
흐름은 단순했다. 디스코드에서 `@ttalkkakthon 심사 {팀이름}` 을 호출하면 봇이 `/api/evaluate?team=...` 으로 프로젝트 데이터를 받아오고, 응답에 동봉된 `evaluationPrompt` 를 Claude Channels에 그대로 던진다. Claude는 GitHub/배포/영상 URL에 직접 접근해서 코드와 실제 결과물까지 본 다음 점수와 코멘트를 만들어내고, 그 결과를 다시 `POST /api/evaluate` 로 DB에 저장한다. 봇이 LLM을 직접 호출하지 않고 프롬프트만 운반하는 구조라, 평가 기준이 바뀌어도 `lib/prompts.ts` 만 갱신하면 됐다.

평가 기준은 다음과 같이 100점 만점으로 잡았다.

| 항목              | 배점 | 핵심 질문                                      |
| ----------------- | ---- | ---------------------------------------------- |
| 아이디어 / 기획력 | 25   | 문제 정의가 명확하고 해결 방안이 실현 가능한가 |
| AI 활용도         | 30   | AI가 보조가 아닌 본질적 기능으로 쓰였는가      |
| 완성도            | 25   | 데모 URL에서 핵심 기능이 실제로 동작하는가     |
| 기술 구현력       | 20   | GitHub 코드 품질·구조·예외 처리가 적절한가     |

여기에 더해 "이웃집 고양이 나비"라는 캐릭터가 진행하는 **냥심사**(15점 만점, 별도 보너스)를 따로 만들었다. 첫인상, 시각적 자극도, 졸음 유발 지수, 쫓고 싶은 요소를 3점 척도로 채점하는 예능용 트랙이다. 본심사와 DB 컬럼(`promptFeedback` / `catFeedback`)을 분리해서 점수에는 영향을 안 주되, 결과 페이지에서 따로 노출했다.
...
<div className="flex justify-center items-start gap-4">
  <img
    src="/images/posts/ttalkkakthon-organizer-retrospective/004.webp"
    alt="나만의 4컷 기기"
    className="h-48 md:h-80 w-auto rounded-lg"
  />
  <img
    src="/images/posts/ttalkkakthon-organizer-retrospective/005.webp"
    alt="나만의 4컷 결과물"
    className="h-48 md:h-80 w-auto rounded-lg"
  />
</div>
