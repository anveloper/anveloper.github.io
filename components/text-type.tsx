...
export const TextType = ({
  ...
  const renderText = () => {
    ...
    const idx = displayedText.indexOf(highlightText);
    if (idx === -1) {
      return <span style={{ color: getCurrentTextColor() || "inherit" }}>{displayedText}</span>;
    }
    const before = displayedText.slice(0, idx);
    const match = displayedText.slice(idx, idx + highlightText.length);
    const after = displayedText.slice(idx + highlightText.length);
    return (
