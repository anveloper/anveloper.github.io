...
export const RecentPosts = ({ items, basePath }: RecentPostsProps) => {
  ...
  return (
    <section aria-labelledby="recent-posts-heading" className="mt-12 border-t border-border pt-6">
      <h2 id="recent-posts-heading" className="text-sm font-medium text-foreground mb-4">
        최근 글
      </h2>
      <ul className="space-y-2">
        {items.map((item) => (
          <li key={item.slug}>
            ...
            >
              <time dateTime={item.date.replace(/\./g, "-")} className="text-xs shrink-0 tabular-nums">
                {item.date}
              </time>
